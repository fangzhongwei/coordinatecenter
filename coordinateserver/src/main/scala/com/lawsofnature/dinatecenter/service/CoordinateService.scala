...
import com.lawsofnature.dinatecenter.repo._
...
class CoordinateServiceImpl @Inject()(coordinateRepository: CoordinateRepository, redisClientTemplate: RedisClientTemplate, gameService: GameService) extends CoordinateService {
  ...
  def startGame(traceId: String, room: TmRoomRow, table: TmTableRow, gameType: Int, seats: Seq[TmSeatRow]) = {
    ...
    gameService.createGame(traceId, gameType, seats1, seats2, seats3)
  ...
  override def consume(data: Array[Byte]): Future[Unit] = {
    ...
    Future {
      val joinRequest: JoinRequest = GameCenterRequest.JoinRequest.parseFrom(data)
      val memberId: Long = joinRequest.getMemberId
      val gameType: Int = joinRequest.getGameType
      findSeat("", memberId, gameType)
    }
    promise.future
  }
}
